...
- [ ] Fase 6 — Statistiche e UI — **codice scritto, test unitari/DOM verdi** (settings, features/stats, contatore marker in dom-scheduler, core/ui/{bars,pill,panel,welcome,debug-overlay}, i18n IT/EN estesi, `time-tracker`/`update-check` agganciati a `main.ts`). **Manca la verifica visiva** su emulatore/iPhone (posizione pillola sopra la nav bar, tema chiaro/scuro, `isFullscreenVideo` su YouTube — vedi TODO in `src/main.ts`, mai verificato dal vivo)
...
3. ~~`docs/TESTING-iphone.md`~~ — **bozza scritta in anticipo il 2026-09-18** (checklist §7.3/§7.4 + punti spike ancora aperti), passi su Orion + Tampermonkey, **non** Impostazioni › Safari › Estensioni. Da eseguire ed eventualmente correggere quando l'utente fa il giro di test su iPhone.
